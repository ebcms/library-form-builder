...
<div class="mb-3">
    ...
    <div>
        ...
        <div class="form-check {if $inline}form-check-inline{/if}">
            <input class="form-check-input" type="radio" onclick="$('#{$key}').removeClass('d-none').siblings().addClass('d-none')" name="{$name}" id="field_{:md5($name .'~'. $vo['value'])}" value="{$vo['value']}" {if isset($vo['disabled']) && $vo['disabled']}disabled{/if} {:$vo['value']==$value?' checked':''}>
