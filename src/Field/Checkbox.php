...
<div class="mb-3">
    ...
    <div>
        ...
        <div class="form-check {if $inline}form-check-inline{/if}">
            <input class="form-check-input" type="checkbox" name="{$name}[]" id="field_{:md5($name .'~'. $vo['value'])}" value="{$vo['value']}" {if isset($vo['disabled']) && $vo['disabled']}disabled{else}{:in_array($vo['value'], $value)?'checked':''}{/if}>
